...
DECLARE_CLASS_CORDL_INTERFACES(BSML, FloatingScreenHandle, UnityEngine::MonoBehaviour, std::vector<Il2CppClass*>({classof(::UnityEngine::EventSystems::IPointerEnterHandler), classof(::UnityEngine::EventSystems::IPointerExitHandler)}),
    DECLARE_FIELDS(
        DECLARE_INSTANCE_FIELD(UnityEngine::Material, originalMaterial);
        DECLARE_INSTANCE_FIELD(UnityEngine::MeshRenderer, renderer);
    );

    FIELD_ACCESSOR(originalMaterial);
    FIELD_ACCESSOR(renderer);
