...
namespace BSML {
    ...
    std::shared_ptr<BSMLParser> parse_and_construct(std::string_view str, UnityEngine::Transform parent, bs_hook::Il2CppWrapperType host = nullptr);
    ...
    namespace Register {
        ...
        /// @return true if successful, false if failed
        bool RegisterSettingsMenu(std::string_view name, std::string_view content_key, bs_hook::Il2CppWrapperType host, bool enableExtraButtons = false);
        ...
        /// @return true if successful, false if failed
        template<typename T>
        requires(std::is_base_of_v<HMUI::ViewController>)
        ...
        /// @return true if successful, false if failed
        bool RegisterGameplaySetupTab(std::string_view name, std::string_view content_key, bs_hook::Il2CppWrapperType host, MenuType menuType = MenuType::All);
        ...
        requires(std::is_base_of_v<UnityEngine::MonoBehaviour, T>)
